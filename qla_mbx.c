	...
	struct vp_rpt_id_entry_24xx *rptid_entry)
{
	...
	if (rptid_entry->format == 0) {
		...
		if (fcport) {
			...
			/*
			 * For the case where remote port sending PRLO, FW sends up
			 * RIDA Format 2 as an indication of session loss.  In other
			 * word, FW state change from PRLI complete back to PLOGI
			 * complete.  Delete the session and let relogin drive the
			 * reconnect.
			 */
	...
	uint16_t *mbx_in, uint16_t *mbx_out)
{
	...
	if (rval != QLA_SUCCESS)
		...
		ql_dbg (ql_dbg_mbx + ql_dbg_verbose, vha, 0xf0a3, \
			"Done %s.\n", __func__);
