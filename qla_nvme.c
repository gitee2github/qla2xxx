...
int qla_nvme_register_hba(struct scsi_qla_host *vha)
{
	...
	if (ql2xnvme_queues < MIN_NVME_HW_QUEUES) {
		ql_log(ql_log_warn, vha, 0xfffd,
		    "ql2xnvme_queues=%d is lower than minimum queues: %d "
		    "Resetting ql2xnvme_queues to:%d\n", ql2xnvme_queues,
		    MIN_NVME_HW_QUEUES, DEF_NVME_HW_QUEUES);

		ql2xnvme_queues = DEF_NVME_HW_QUEUES;
	}
	else if (ql2xnvme_queues > (ha->max_qpairs - 1)) {
		ql_log(ql_log_warn, vha, 0xfffd,
		    "ql2xnvme_queues=%d is greater than avalable IRQs:%d "
		    "Resetting ql2xnvme_queues to:%d\n", ql2xnvme_queues,
		    (ha->max_qpairs - 1), (ha->max_qpairs - 1));
